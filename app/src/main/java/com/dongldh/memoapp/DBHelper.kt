...
class DBHelper(context: Context): SQLiteOpenHelper(context, "memoDB", null, 1) {
    override fun onCreate(db: SQLiteDatabase?) {
        ...
        val memoInitSQL1 = "insert into t_memo (title, content, date) values ('환영합니다!', '똑똑노트는 실생활에서 사용되는 각종 정보를 쉽고 편하게 정리할 수 있도록 만든 어플입니다." +
                "\n\n똑똑노트는 무료인가요? 네. 무료입니다. 대신 주위 분들에게 많이 많이 알려주세요! 개발자도 먹고 살아야 하니까요!!', '${System.currentTimeMillis()}')"
        val memoInitSQL2 = "insert into t_memo (title, content, date) values ('똑똑노트 사용법', '알아서 사용하세용', '${System.currentTimeMillis()}')"
        val memoInitSQL3 = "insert into t_memo (title, content, date) values ('스마트폰 사용 시 유용한 팁', '계산기 어플을 다운받아 핸드폰에 내장된 기본 계산기 어플의 테마를 바꿔보세요!', '${System.currentTimeMillis()}')"
        val memoInitSQL4 = "insert into t_memo (title, content, date) values ('핸드폰 변경 시 자료 옮기는 법', '추후 업데이트 예정', '${System.currentTimeMillis()}')"
